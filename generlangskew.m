function SK = generlangskew(lambda)

% function SK = generlangskew(lambda)
%
%   example call: generlangskew([1 2 3]); 
%
% skew of generalized Erlang (or hypoexponential) random variables 
%
...
if length(unique(lambda)) == 1, error(['generlangskew.m: WARNING! lambdas must be different. must be different... else generlangpdf -> erlangpdf']); end

SK = 2.*sum(1./lambda.^3)./(sum(1./lambda.^2).^(3/2));