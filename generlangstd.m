function SD = generlangstd(lambda)

% function SD = generlangstd(lambda)
%
%   example call: generlangstd([1 2 3])
%
% standard deviation of generalized Erlang (or hypoexponential) random variable 
%
...
% INPUT CHECKING
if length(unique(lambda)) == 1, error('generlangstd.m: WARNING! lambdas must be different... else generlangpdf -> erlangpdf OR if k = 1 -> exppdf'); end


SD = sqrt( generlangvar(lambda) );